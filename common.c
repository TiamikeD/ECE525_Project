...
// ========================================================================================================
// ========================================================================================================
// Compute the number of bytes in the Chlng packet based on the mask.

int ComputeChlngPacketSize(int max_string_len, int num_required_nonce_bytes, int num_required_PNDiffs,
   int num_SF_bytes, int CH_LLK_num_bytes, int num_iterations, unsigned char mask[2])
   {
   int packet_num_bytes = 0;

// Chip_num
   if ( GetBitFromByte(mask[0], 0) == 1 )
      packet_num_bytes += 4;

// CH_LLK_num_bytes
   if ( GetBitFromByte(mask[0], 1) == 1 )
      packet_num_bytes += 4;

// CH_LLK
   if ( GetBitFromByte(mask[0], 2) == 1 )
      packet_num_bytes += CH_LLK_num_bytes;

// v_a
   if ( GetBitFromByte(mask[0], 3) == 1 )
      packet_num_bytes += 4;

// XOR_nonce
   if ( GetBitFromByte(mask[0], 4) == 1 )
      packet_num_bytes += num_required_nonce_bytes;

// num_iterations
   if ( GetBitFromByte(mask[0], 5) == 1 )
      packet_num_bytes += 4;

// XHD
   if ( GetBitFromByte(mask[0], 6) == 1 )
      packet_num_bytes += num_iterations * num_required_PNDiffs/8;

// SF
   if ( GetBitFromByte(mask[0], 7) == 1 )
      packet_num_bytes += num_iterations * num_SF_bytes;

   return packet_num_bytes;
   }


// ========================================================================================================
// ========================================================================================================
// Assemble a Chlng packet. NOTE that we assume space within Chlng_packet has been allocated in the caller. 

void AssembleChlngPacket(int max_string_len, int num_required_nonce_bytes, int num_required_PNDiffs,
   int num_SF_bytes, unsigned char *Chlng_packet, int Chlng_packet_size, int chip_num, int CH_LLK_num_bytes, 
   unsigned char *CH_LLK, unsigned int v_a, unsigned char *XOR_nonce, int num_iterations, unsigned char *XHD, 
   signed char *SF, unsigned char *mask, int check_size)
   {
   int pbyte_num, fbyte_num;

printf("AssembleChlngPacket(): BEGIN\n"); fflush(stdout); 
#ifdef DEBUG
#endif

// The 0th potential field is chip_num.
   pbyte_num = 0;
   if ( GetBitFromByte(mask[0], 0) == 1 )
      {
      if ( chip_num < 0 )
         { printf("ERROR: AssembleChlngPacket(): chip_num < 0!\n"); exit(EXIT_FAILURE); }
      for ( fbyte_num = 0; fbyte_num < 4; pbyte_num++, fbyte_num++ )
         {
         if ( pbyte_num >= Chlng_packet_size )
            { printf("ERROR: AssembleChlngPacket(): Load Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
         Chlng_packet[pbyte_num] = (unsigned char)(chip_num >> (fbyte_num << 3)); 
         }
      }

// The 1st potential field is CH_LLK_num_bytes.
   if ( GetBitFromByte(mask[0], 1) == 1 )
      {
      if ( CH_LLK_num_bytes <= 0 )
         { printf("ERROR: AssembleChlngPacket(): CH_LLK_num_bytes <= 0!\n"); exit(EXIT_FAILURE); }
      for ( fbyte_num = 0; fbyte_num < 4; pbyte_num++, fbyte_num++ )
         {
         if ( pbyte_num >= Chlng_packet_size )
            { printf("ERROR: AssembleChlngPacket(): Load Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
         Chlng_packet[pbyte_num] = (unsigned char)(CH_LLK_num_bytes >> (fbyte_num << 3));
         }
      }

// The 2nd field is CH_LLK.
   if ( GetBitFromByte(mask[0], 2) == 1 )
      {
      if ( CH_LLK_num_bytes <= 0 || CH_LLK == NULL )
         { printf("ERROR: AssembleChlngPacket(): CH_LLK_num_bytes <= 0 OR CH_LLK is NULL!\n"); exit(EXIT_FAILURE); }

      if ( pbyte_num + CH_LLK_num_bytes > Chlng_packet_size )
         { printf("ERROR: AssembleChlngPacket(): Load Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
      memcpy(&(Chlng_packet[pbyte_num]), CH_LLK, CH_LLK_num_bytes);
      pbyte_num += CH_LLK_num_bytes;
      }

// The 3rd field is v_a.
   if ( GetBitFromByte(mask[0], 3) == 1 )
      for ( fbyte_num = 0; fbyte_num < 4; pbyte_num++, fbyte_num++ )
         {
         if ( pbyte_num >= Chlng_packet_size )
            { printf("ERROR: AssembleChlngPacket(): Load Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
         Chlng_packet[pbyte_num] = (unsigned char)(v_a >> (fbyte_num << 3));
         }

// The 4th field is XOR_nonce.
   if ( GetBitFromByte(mask[0], 4) == 1 )
      {
      if ( XOR_nonce == NULL )
         { printf("ERROR: AssembleChlngPacket(): XOR_nonce is NULL!\n"); exit(EXIT_FAILURE); }

      if ( pbyte_num + num_required_nonce_bytes > Chlng_packet_size )
         { printf("ERROR: AssembleChlngPacket(): Load Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
      memcpy(&(Chlng_packet[pbyte_num]), XOR_nonce, num_required_nonce_bytes);
      pbyte_num += num_required_nonce_bytes;
      }

// The 5th field is num_iterations.
   if ( GetBitFromByte(mask[0], 5) == 1 )
      {
      if ( num_iterations <= 1 )
         { printf("ERROR: AssembleChlngPacket(): num_iterations <= 1!\n"); exit(EXIT_FAILURE); }
      for ( fbyte_num = 0; fbyte_num < 4; pbyte_num++, fbyte_num++ )
         {
         if ( pbyte_num >= Chlng_packet_size )
            { printf("ERROR: AssembleChlngPacket(): Load Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
         Chlng_packet[pbyte_num] = (unsigned char)num_iterations >> (fbyte_num << 3);
         }
      }

// The 6th field is XHD.
   if ( GetBitFromByte(mask[0], 6) == 1 )
      {
      if ( XHD == NULL )
         { printf("ERROR: AssembleChlngPacket(): XHD is NULL!\n"); exit(EXIT_FAILURE); }

      if ( pbyte_num + num_iterations * num_required_PNDiffs/8 > Chlng_packet_size )
         { printf("ERROR: AssembleChlngPacket(): Load Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
      memcpy(&(Chlng_packet[pbyte_num]), XHD, num_iterations * num_required_PNDiffs/8);
      pbyte_num += num_iterations * num_required_PNDiffs/8;
      }
...
// The 7th field is SF.
   if ( GetBitFromByte(mask[0], 7) == 1 )
      {
      if ( SF == NULL )
         { printf("ERROR: AssembleChlngPacket(): SF is NULL!\n"); exit(EXIT_FAILURE); }

      if ( pbyte_num + num_iterations * num_SF_bytes > Chlng_packet_size )
         { printf("ERROR: AssembleChlngPacket(): Load Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
      memcpy(&(Chlng_packet[pbyte_num]), SF, num_iterations * num_SF_bytes);
      pbyte_num += num_iterations * num_SF_bytes;
      }

// This is an error condition.
   if ( check_size == 1 && pbyte_num > Chlng_packet_size )
      { printf("ERROR: AssembleChlngPacket(): EXPECTED packet length to be %d\tLoaded %d!\n", Chlng_packet_size, pbyte_num); exit(EXIT_FAILURE); }

if ( check_size == 1 && pbyte_num < Chlng_packet_size )
   { printf("WARNING: AssembleChlngPacket(): EXPECTED packet length to be %d\tONLY Loaded %d!\n", Chlng_packet_size, pbyte_num); fflush(stdout); }
#ifdef DEBUG
#endif

#ifdef DEBUG
printf("AssembleChlngPacket(): LLK fields:\n"); fflush(stdout);
printf("\tChip_num %d\tCH_LLK_num_bytes %d\n", chip_num, CH_LLK_num_bytes); fflush(stdout);
if ( CH_LLK != NULL )
   PrintHeaderAndHexVals("\tCH_LLK:\n", CH_LLK_num_bytes, CH_LLK, 32);
printf("\tv_a %u\n", v_a); fflush(stdout);
if ( XOR_nonce != NULL )
   PrintHeaderAndHexVals("\tXOR_nonce:\n", num_required_nonce_bytes, XOR_nonce, 32);
printf("\tnum_iterations %d\n", num_iterations); fflush(stdout);
if ( XHD != NULL )
   PrintHeaderAndHexVals("\tXHD:\n", num_iterations * num_required_PNDiffs/8, XHD, 32);
if ( SF != NULL )
   PrintHeaderAndHexVals("\tSF:\n", num_iterations * num_SF_bytes, (unsigned char *)SF, 32);
fflush(stdout);
#endif

printf("AssembleChlngPacket(): DONE\n"); fflush(stdout); 
#ifdef DEBUG
#endif
...
// ========================================================================================================
// ========================================================================================================
// Extract the fields from the Chlng_packet. Note that we allocate space here as needed for the blob fields, 
// CH_LLK, XOR_nonce, XHD and SF, the size of some of these depending on num_iterations field in the packet 
// itself.

void ExtractChlngPacket(int max_string_len, int num_required_nonce_bytes, int num_required_PNDiffs,
   int num_SF_bytes, unsigned char *Chlng_packet, int Chlng_packet_size, int *chip_num_ptr, 
   int *CH_LLK_num_bytes_ptr, unsigned char **CH_LLK_ptr, unsigned int *v_a_ptr, unsigned char **XOR_nonce_ptr, 
   int *num_iterations_ptr, unsigned char **XHD_ptr, signed char **SF_ptr, unsigned char *mask)
   {
   int pbyte_num, fbyte_num;

#ifdef DEBUG
printf("ExtractChlngPacket(): BEGIN\n"); fflush(stdout); 
#endif

// Sanity check.
   if ( Chlng_packet == NULL || Chlng_packet_size <= 0 )
      { printf("ERROR: ExtractChlngPacket(): Chlng_packet is NULL or Chlng_packet_size is <= 0!\n"); exit(EXIT_FAILURE); }

// The 0th potential field is chip_num.
   pbyte_num = 0;
   if ( GetBitFromByte(mask[0], 0) == 1 )
      {
      if ( chip_num_ptr == NULL )
         { printf("ERROR: ExtractChlngPacket(): 'mask' ERROR with chip_num_ptr!\n"); exit(EXIT_FAILURE); }

      *chip_num_ptr = 0;
      for ( fbyte_num = 0; fbyte_num < 4; pbyte_num++, fbyte_num++ )
         {
         if ( pbyte_num >= Chlng_packet_size )
            { printf("ERROR: AssembleChlngPacket(): Extract Chlng_packet overflow: pbyte_num >= Chlng_packet_size!\n"); exit(EXIT_FAILURE); }
         *chip_num_ptr += (int)(Chlng_packet[pbyte_num] << (fbyte_num << 3)); 
         }
      if ( *chip_num_ptr < 0 )
         { printf("ERROR: ExtractChlngPacket(): Extracted chip_num < 0: %d!\n", *chip_num_ptr); exit(EXIT_FAILURE); }
      }

// The 1st potential field is CH_LLK_num_bytes.
   if ( GetBitFromByte(mask[0], 1) == 1 )
      {
      if ( CH_LLK_num_bytes_ptr == NULL )
         { printf("ERROR: ExtractChlngPacket(): 'mask' ERROR with CH_LLK_num_bytes_ptr!\n"); exit(EXIT_FAILURE); }
 
      *CH_LLK_num_bytes_ptr = 0;
      for ( fbyte_num = 0; fbyte_num < 4; pbyte_num++, fbyte_num++ )
         {
         if ( pbyte_num >= Chlng_packet_size )
            { printf("ERROR: AssembleChlngPacket(): Extract Chlng_packet overflow: pbyte_num >= Chlng_packet_size!\n"); exit(EXIT_FAILURE); }
         *CH_LLK_num_bytes_ptr += (int)(Chlng_packet[pbyte_num] << (fbyte_num << 3));
         }
      if ( *CH_LLK_num_bytes_ptr <= 0 )
         { printf("ERROR: ExtractChlngPacket(): Extracted CH_LLK_num_bytes <= 0: %d!\n", *CH_LLK_num_bytes_ptr); exit(EXIT_FAILURE); }
      }
...
// The 2nd field is CH_LLK.
   if ( GetBitFromByte(mask[0], 2) == 1 )
      {
      if ( CH_LLK_ptr == NULL )
         { printf("ERROR: ExtractChlngPacket(): 'mask' ERROR with CH_LLK!\n"); exit(EXIT_FAILURE); }

// Free up any existing storage.
      if ( *CH_LLK_ptr != NULL )
         free(*CH_LLK_ptr);

      if ( *CH_LLK_num_bytes_ptr == 0 )
         { printf("ERROR: ExtractChlngPacket(): CH_LLK_num_bytes is 0!\n"); exit(EXIT_FAILURE); }
      if ( (*CH_LLK_ptr = (unsigned char *)calloc(*CH_LLK_num_bytes_ptr, sizeof(unsigned char))) == NULL )
         { printf("ERROR: ExtractChlngPacket(): Failed to allocate storage for CH_LLK!\n"); exit(EXIT_FAILURE); }

      if ( pbyte_num + *CH_LLK_num_bytes_ptr > Chlng_packet_size )
         { printf("ERROR: AssembleChlngPacket(): Extract Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
      memcpy(*CH_LLK_ptr, &(Chlng_packet[pbyte_num]), *CH_LLK_num_bytes_ptr);
      pbyte_num += *CH_LLK_num_bytes_ptr;

#ifdef DEBUG
PrintHeaderAndHexVals("\tExtractChlngPacket(): EXTRACTED CH_LLK:\n", *CH_LLK_num_bytes_ptr, *CH_LLK_ptr, 32);
#endif
      }
...
// The 3rd field is v_a.
   if ( GetBitFromByte(mask[0], 3) == 1 )
      {
      if ( v_a_ptr == NULL )
         { printf("ERROR: ExtractChlngPacket(): 'mask' ERROR with v_a_ptr!\n"); exit(EXIT_FAILURE); }
      ...
      *v_a_ptr = 0;
      for ( fbyte_num = 0; fbyte_num < 4; pbyte_num++, fbyte_num++ )
         ...
         if ( pbyte_num >= Chlng_packet_size )
            { printf("ERROR: AssembleChlngPacket(): Extract Chlng_packet overflow: pbyte_num >= Chlng_packet_size!\n"); exit(EXIT_FAILURE); }
         *v_a_ptr += (Chlng_packet[pbyte_num] << (fbyte_num << 3));
         }
      }
...
// Sanity check.
      if ( XOR_nonce_ptr == NULL )
         { printf("ERROR: ExtractChlngPacket(): 'mask' ERROR with XOR_nonce_ptr!\n"); exit(EXIT_FAILURE); }

      if ( *XOR_nonce_ptr != NULL )
         free(*XOR_nonce_ptr);
      if ( (*XOR_nonce_ptr = (unsigned char *)calloc(num_required_nonce_bytes, sizeof(unsigned char))) == NULL )
         { printf("ERROR: ExtractChlngPacket(): Failed to allocate storage for XOR_nonce!\n"); exit(EXIT_FAILURE); }

      if ( pbyte_num + num_required_nonce_bytes > Chlng_packet_size )
         { printf("ERROR: AssembleChlngPacket(): Extract Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
      memcpy(*XOR_nonce_ptr, &(Chlng_packet[pbyte_num]), num_required_nonce_bytes);
      pbyte_num += num_required_nonce_bytes;
      }
...
// The 5th field is num_iterations.
   if ( GetBitFromByte(mask[0], 5) == 1 )
      ...
      if ( num_iterations_ptr == NULL )
         { printf("ERROR: ExtractChlngPacket(): 'mask' ERROR with num_iterations_ptr!\n"); exit(EXIT_FAILURE); }
      ...
      *num_iterations_ptr = 0;
      for ( fbyte_num = 0; fbyte_num < 4; pbyte_num++, fbyte_num++ )
         ...
         if ( pbyte_num >= Chlng_packet_size )
            { printf("ERROR: AssembleChlngPacket(): Extract Chlng_packet overflow: pbyte_num >= Chlng_packet_size!\n"); exit(EXIT_FAILURE); }
         *num_iterations_ptr += (Chlng_packet[pbyte_num] << (fbyte_num << 3)); 
         }
      if ( *num_iterations_ptr <= 0 )
         { printf("ERROR: ExtractChlngPacket(): Extracted num_iterations <= 0: %d!\n", *num_iterations_ptr); exit(EXIT_FAILURE); }
      }
...
// The 6th field is XHD.
   if ( GetBitFromByte(mask[0], 6) == 1 )
      {
      if ( XHD_ptr == NULL )
         { printf("ERROR: ExtractChlngPacket(): 'mask' ERROR with XHD_ptr!\n"); exit(EXIT_FAILURE); }

      if ( *XHD_ptr != NULL )
         free(*XHD_ptr); 
      if ( (*XHD_ptr = (unsigned char *)calloc(*num_iterations_ptr * num_required_PNDiffs/8, sizeof(unsigned char))) == NULL )
         { printf("ERROR: ExtractChlngPacket(): Failed to allocate storage for XHD!\n"); exit(EXIT_FAILURE); }

      if ( pbyte_num + *num_iterations_ptr * num_required_PNDiffs/8 > Chlng_packet_size )
         { printf("ERROR: AssembleChlngPacket(): Extract Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
      memcpy(*XHD_ptr, &(Chlng_packet[pbyte_num]), *num_iterations_ptr * num_required_PNDiffs/8);
      pbyte_num += *num_iterations_ptr * num_required_PNDiffs/8;
      }
...
// The 7th field is SF.
   if ( GetBitFromByte(mask[0], 7) == 1 )
      {
      if ( SF_ptr == NULL )
         { printf("ERROR: ExtractChlngPacket(): 'mask' ERROR with SF_ptr!\n"); exit(EXIT_FAILURE); }

      if ( *SF_ptr != NULL )
         free(*SF_ptr); 
      if ( (*SF_ptr = (signed char *)calloc(*num_iterations_ptr * num_SF_bytes, sizeof(signed char))) == NULL )
         { printf("ERROR: ExtractChlngPacket(): Failed to allocate storage for SF!\n"); exit(EXIT_FAILURE); }

      if ( pbyte_num + *num_iterations_ptr * num_SF_bytes > Chlng_packet_size )
         { printf("ERROR: AssembleChlngPacket(): Extract Chlng_packet overflow!\n"); exit(EXIT_FAILURE); }
      memcpy(*SF_ptr, &(Chlng_packet[pbyte_num]), *num_iterations_ptr * num_SF_bytes);
      pbyte_num += *num_iterations_ptr * num_SF_bytes;
      }

#ifdef DEBUG
printf("ExtractChlngPacket(): LLK fields:\n"); fflush(stdout);
printf("\tChip_num %d\tCH_LLK_num_bytes %d\n", *chip_num_ptr, *CH_LLK_num_bytes_ptr); fflush(stdout);
if ( CH_LLK_ptr != NULL )
   PrintHeaderAndHexVals("\tCH_LLK:\n", *CH_LLK_num_bytes_ptr, *CH_LLK_ptr, 32);
printf("\tv_a %u\n", *v_a_ptr); fflush(stdout);
if ( XOR_nonce_ptr != NULL )
   PrintHeaderAndHexVals("\tXOR_nonce:\n", num_required_nonce_bytes, *XOR_nonce_ptr, 32);
printf("\tnum_iterations %d\n", *num_iterations_ptr); fflush(stdout);
if ( XHD_ptr != NULL )
   PrintHeaderAndHexVals("\tXHD:\n", *num_iterations_ptr * num_required_PNDiffs/8, *XHD_ptr, 32);
if ( SF_ptr != NULL )
   PrintHeaderAndHexVals("\tSF:\n", *num_iterations_ptr * num_SF_bytes, (unsigned char *)*SF_ptr, 32);
fflush(stdout);
#endif

#ifdef DEBUG
printf("ExtractChlngPacket(): DONE\n"); fflush(stdout); 
#endif
